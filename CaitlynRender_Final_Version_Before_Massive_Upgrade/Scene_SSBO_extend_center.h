...
struct Scene
{
	...
	void gpu_data(string shader_direction)
	{
		...
		//glBufferData(GL_SHADER_STORAGE_BUFFER, sizeof(vec3) * vertices.size(), vertices.data(), GL_STATIC_DRAW);
		...
		*/
		...
		path_trace_shader = new Shader(shader_direction + "path_trace.vs", shader_direction + "optimize_path_trace_ssbo_clean_extend_center.fs");
		...
		path_trace_shader->setInt("tree_height", sbvh.tree_height);
