...
float hit_bbox(Ray r, vec3 center, vec3 extend, vec3 invdir, vec3 abs_invdir, out float tl)
{
	// shift origin once
    /*vec3 t1 = (bmin - r.o) * invdir;
    vec3 t2 = (bmax - r.o) * invdir;
	...
	*/
	...
	vec3 t = (center - r.o) * invdir;

	vec3 e = extend * abs_invdir;

	tl = max(max(t.x - e.x, t.y - e.y), t.z - e.z);
	float th = min(min(t.x + e.x, t.y + e.y), t.z + e.z);
...
bool trace_bvh(Ray r, out HitRecord rec)
{
	...
	//const vec3 abs_invdir = abs(invdir);
...
bool trace_bvh_branch_less(Ray r, out HitRecord rec)
{
	...
	while(ind > -1)
	{
		...
		if(range == 0)
		{
			...
			//best!
			float th1 = hit_bbox(r, bvh[leftIndex * 2].xyz, bvh[leftIndex * 2 + 1].xyz, invdir, abs_invdir,  tl1);
			float th2 = hit_bbox(r, bvh[rightIndex * 2].xyz, bvh[rightIndex * 2 + 1].xyz, invdir, abs_invdir, tl2);
