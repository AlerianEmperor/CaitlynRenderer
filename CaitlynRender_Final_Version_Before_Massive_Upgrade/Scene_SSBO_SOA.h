...
struct Light
{
	...
	//Light(vec4 p_, vec4 u_, vec4 v_, vec4 e_, vec4 n_, vec4 area_pdf_) : p(p_), u(u_), v(v_), e(e_), n(n_), area_pdf(area_pdf_) {}
};
...
struct Light4
{
	...
	Light4() {}
	//Light(vec3 p_, vec3 u_, vec3 v_, vec3 e_, vec3 n_, vec3 area_pdf_) : p(p_), u(u_), v(v_), e(e_), n(n_), area_pdf(area_pdf_) {}
	Light4(vec4 p_, vec4 u_, vec4 v_, vec4 e_, vec4 n_, vec4 area_pdf_) : p(p_), u(u_), v(v_), e(e_), n(n_), area_pdf(area_pdf_) {}
};
...
struct Scene
{
			...
			else if (strncmp(t, "f", 1) == 0)
			{
				...
				for (int i = 0; i < faces.size(); ++i)
				{
					...
					if (mats[mtl_ind].emission.w != -1)//light
					{
						...
						//Light light(vec4(p0.x, p0.y, p0.z, 0.0f), vec4(u.x, u.y, u.z, 0.0f), vec4(v.x, v.y, v.z, 0.0f), vec4(e.x, e.y, e.z, 0.0f), vec4(n.x, n.y, n.z, 0.0f), vec4(area, 0.0f, 0.0f, 0.0f));
	...
	void gpu_data(string shader_direction)
	{
		...
		for (auto& v : vertices) vertices4.push_back(glm::vec4(v, 1.0f));

		glGenBuffers(1, &vertices_buffer);
		glBindBuffer(GL_SHADER_STORAGE_BUFFER, vertices_buffer);
		glBufferData(GL_SHADER_STORAGE_BUFFER, sizeof(glm::vec4) * vertices4.size(), vertices4.data(), GL_STATIC_DRAW);
		glBindBufferBase(GL_SHADER_STORAGE_BUFFER, 1, vertices_buffer);

		vector<vec4>().swap(vertices4);
		vector<vec3>().swap(vertices);

		std::vector<glm::vec4> normals4;
		normals4.reserve(normals.size());
		for (auto& n : normals) normals4.push_back(glm::vec4(n, 1.0f));

		glCreateBuffers(1, &normals_buffer);
		glNamedBufferData(normals_buffer, sizeof(vec4) * normals4.size(), normals4.data(), GL_STATIC_DRAW);
		glBindBufferBase(GL_SHADER_STORAGE_BUFFER, 2, normals_buffer);

		vector<vec4>().swap(normals4);
		vector<vec3>().swap(normals);


		//std::vector<glm::vec2> texcoords2;
		//texcoords2.reserve(texcoords.size());
		//for (auto& t : texcoords) texcoords2.push_back(t);

		glCreateBuffers(1, &texcoords_buffer);
		glNamedBufferData(texcoords_buffer, sizeof(vec2) * texcoords.size(), texcoords.data(), GL_STATIC_DRAW);
		glBindBufferBase(GL_SHADER_STORAGE_BUFFER, 3, texcoords_buffer);

		//vector<vec4>().swap(normals4);
		vector<vec2>().swap(texcoords);


		glCreateBuffers(1, &triangles_buffer);
		glNamedBufferData(triangles_buffer, sizeof(Triangle) * triangles.size(), triangles.data(), GL_STATIC_DRAW);
		glBindBufferBase(GL_SHADER_STORAGE_BUFFER, 4, triangles_buffer);

		vector<Triangle>().swap(triangles);

		
		glCreateBuffers(1, &mats_buffer);
		glNamedBufferData(mats_buffer, sizeof(Material) * mats.size(), mats.data(), GL_STATIC_DRAW);
		glBindBufferBase(GL_SHADER_STORAGE_BUFFER, 5, mats_buffer);

		vector<Material>().swap(mats);

		vector<Light4> light4s;
		light4s.resize(lights.size());
		int ind = 0;
		for (auto& l : lights)
		{
			//cout << l.p.x<<"  ";
			Light4 light(vec4(l.p.x, l.p.y, l.p.z, 0.0f), vec4(l.u.x, l.u.y, l.u.z, 0.0f), vec4(l.v.x, l.v.y, l.v.z, 0.0f), vec4(l.e.x, l.e.y, l.e.z, 0.0f), vec4(l.n.x, l.n.y, l.n.z, 0.0f), vec4(l.area_pdf.x, l.area_pdf.y, l.area_pdf.z, 0.0f));
			//Light4 light(vec4(l.p.x, l.p.y, l.p.z, 0.0f), vec4(l.u.x, l.u.y, l.u.z, 0.0f), vec4(l.v.x, l.v.y, l.v.z, 0.0f), vec4(l.e.x, l.e.y, l.e.z, 0.0f), vec4(l.n.x, l.n.y, l.n.z, 0.0f), vec4(l.area_pdf.x, 0.0f, 0.0f, 0.0f));

			light4s[ind++] = (light);
			//light4s[1] = (light);
		}
		glCreateBuffers(1, &lights_buffer);
		glNamedBufferData(lights_buffer, sizeof(Light4) * light4s.size(), light4s.data(), GL_STATIC_DRAW);
		glBindBufferBase(GL_SHADER_STORAGE_BUFFER, 6, lights_buffer);


		vector<Light>().swap(lights);
		vector<Light4>().swap(light4s);

		
		glCreateBuffers(1, &bvh_buffer);

		size_t total_bytes = sbvh.bvh_soa.min_x.size() * sizeof(float) * 8; //+
			//sbvh.bvh_soa.start.size() * sizeof(float) * 2;

		glNamedBufferData(bvh_buffer, total_bytes, nullptr, GL_STATIC_DRAW);

		//glNamedBufferStorage(bvh_buffer, total_bytes, nullptr, GL_STATIC_DRAW);

		size_t offset = 0;

		auto upload = [&](auto& vec)
		{
				size_t sz = vec.size() * sizeof(vec[0]);
				glNamedBufferSubData(bvh_buffer, offset, sz, vec.data());
				offset += sz;
		};

		//size_t offset = 0;
		auto uploadf = [&](const vector<float>& vec) {
			size_t sz = vec.size() * sizeof(float);
			glNamedBufferSubData(bvh_buffer, offset, sz, vec.data());
			offset += sz;
			};

		cout << "check sbvh data here: \n";

		cout << "minx : " << sbvh.bvh_soa.min_x.size() << " \n";
		cout << "miny : " << sbvh.bvh_soa.min_y.size() << " \n";
		cout << "minz : " << sbvh.bvh_soa.min_z.size() << " \n";

		cout << "minx : " << sbvh.bvh_soa.max_x.size() << " \n";
		cout << "miny : " << sbvh.bvh_soa.max_y.size() << " \n";
		cout << "minz : " << sbvh.bvh_soa.max_z.size() << " \n";

		cout << "start : " << sbvh.bvh_soa.start.size() << " \n";
		cout << "range : " << sbvh.bvh_soa.range.size() << " \n";
		
		cout << "nodeCount : " << sbvh.nodeCount << " \n";

		for (int i = 0; i < sbvh.bvh_soa.min_x.size(); ++i)
		{
			cout << sbvh.bvh_soa.min_x[i] << " " << sbvh.bvh_soa.min_y[i] << " " << sbvh.bvh_soa.min_z[i] << "\n";
		}

		for (int i = 0; i < sbvh.bvh_soa.start.size(); ++i)
		{
			cout << sbvh.bvh_soa.start[i] << " " << sbvh.bvh_soa.range[i] << "\n";
		}

		//glMemoryBarrier(GL_SHADER_STORAGE_BARRIER_BIT);

		uploadf(sbvh.bvh_soa.min_x);
		uploadf(sbvh.bvh_soa.min_y);
		uploadf(sbvh.bvh_soa.min_z);

		uploadf(sbvh.bvh_soa.max_x);
		uploadf(sbvh.bvh_soa.max_y);
		uploadf(sbvh.bvh_soa.max_z);

		//upload(sbvh.bvh_soa.start);
		//upload(sbvh.bvh_soa.range);

		int nodeCount = (int)sbvh.bvh_soa.min_x.size();

		// convert start/range -> float arrays
		vector<float> start_f(nodeCount), range_f(nodeCount);
		for (int i = 0; i < nodeCount; ++i) {
			start_f[i] = float(sbvh.bvh_soa.start[i]);
			range_f[i] = float(sbvh.bvh_soa.range[i]);
		}
		uploadf(start_f);
		uploadf(range_f);

		nodeCount = (int)sbvh.bvh_soa.min_x.size();
		sbvh.nodeCount = nodeCount; // n?u b?n có tr??ng ?ó
		path_trace_fade_shader->use();
		path_trace_fade_shader->setInt("nodeCount", nodeCount);
		...
		std::cout << "total_bytes expected = " << total_bytes << "\n";
		std::cout << "offset after uploads = " << offset << "\n";

		vector<float> tmp(nodeCount * 8);
		glGetNamedBufferSubData(bvh_buffer, 0, tmp.size() * sizeof(float), tmp.data());
		printf("first node floats:\n");
		for (int i = 0; i < 8; i++) {
			printf("%02d: %f\n", i, tmp[i]);
		}
		// start/range are at indices 6 and 7 per-node
		printf("start0 (as float) = %f, range0 (as float) = %f\n", tmp[nodeCount * 6 + 0], tmp[nodeCount * 7 + 0]);
		printf("start0 as int = %d, range0 as int = %d\n", (int)tmp[nodeCount * 6 + 0], (int)tmp[nodeCount * 7 + 0]);

		//glBindBufferBase(GL_SHADER_STORAGE_BUFFER, 7, bvh_buffer);

		//glNamedBufferData(bvh_buffer, sizeof(FlatNode) * flat_nodes.size(), flat_nodes.data(), GL_STATIC_DRAW);
		//glBindBufferBase(GL_SHADER_STORAGE_BUFFER, 7, bvh_buffer);
		...
		path_trace_shader = new Shader(shader_direction + "path_trace.vs", shader_direction + "optimize_path_trace_ssbo_soa.fs");
		...
		path_trace_shader->setInt("nodeCount", sbvh.nodeCount);

		cout << "node count sbvh: " << sbvh.nodeCount << "\n";
