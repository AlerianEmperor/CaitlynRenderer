...
uniform isamplerBuffer texcoord_index_tex;
...
float hit_bbox(Ray r, vec3 bmin, vec3 bmax, vec3 invdir, inout float tl)
{
...
{
	ivec4 triIndex = texelFetch(triangles_tex, 2 * i);
	...
	uvt.z = dot(e1, qv);//t
	//float det = dot(e0, pv);
	float inv_det = 1.0f / dot(e0, pv);//det;
...
bool hit_triangle_no_rec(Ray r, int i, float max_t)
{
	...
	uvt.z = dot(e1, qv);//t
	float inv_det = 1.0f / dot(e0, pv);//1.0f / det;

	uvt.xyz = uvt.xyz * inv_det;
	uvt.w = 1.0 - uvt.x - uvt.y;//u + v >= det

		
	return (all(greaterThanEqual(uvt, vec4(0.0f))) && uvt.z < max_t);
}
...
void compute_normal_texcoord_and_material(inout HitRecord rec)
{
	...
	ivec4 vn = texelFetch(triangles_tex, 2 * triangle_ind + 1);
	...
	if(mat.tex_ind.x != -1)
	{
		...
		ivec4 vt = texelFetch(texcoord_index_tex, triangle_ind );
		...
		mat.albedo.xyz = pow(texture(albedo_textures, vec3(rec.texcoord, mat.tex_ind.x)).xyz, vec3(2.2f));	
	}
	rec.mat = mat;
}
...
bool trace_bvh(Ray r, out HitRecord rec)
{
	...
	while(ind > -1)
	{
		...
		if(bmax.w == 0)
		{
			...
			bool left  = th1 >= 0.0f && th1 >= tl1 && tl1 < t;
			bool right = th2 >= 0.0f && th2 >= tl2 && tl2 < t;
