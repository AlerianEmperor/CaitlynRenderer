...
struct Scene
{
	...
	void gpu_data(string shader_direction)
	{
		...
		glNamedBufferStorage(bvh_buffer, sbvh.soa.size() * sizeof(float), sbvh.soa.data(), 0);
		...
		path_trace_shader = new Shader(shader_direction + "path_trace.vs", shader_direction + "optimize_path_trace_ssbo_soa_2.fs");
		...
		path_trace_shader->setInt("node_count", node_count);
