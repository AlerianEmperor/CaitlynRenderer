...
float hit_bbox(Ray r, vec3 bmin, vec3 bmax, vec3 invdir, vec3 dor, out float tl)
{
	
	bmin = bmin * invdir + dor;
	bmax = bmax * invdir + dor;
...
bool trace_bvh(Ray r, out HitRecord rec)
{
	...
	while(ind > -1)
	{
		...
		if(bmax.w == 0)
		{
			...
			float th1 = hit_bbox(r.o, bvh[leftIndex * 2].xyz, bvh[leftIndex * 2 + 1].xyz, invdir, tl1);
			float th2 = hit_bbox(r.o, bvh[rightIndex * 2].xyz, bvh[rightIndex * 2 + 1].xyz, invdir, tl2);
			...
			/*bool hitL = left  && (tl1 <= t);
			bool hitR = right && (tl2 <= t);

			if (hitL && hitR) {
   
			bool chooseLeft = (tl1 < tl2);

   
			stk[ptr++] = chooseLeft ? rightIndex : leftIndex;

    
			ind = chooseLeft ? leftIndex : rightIndex;
			}
			else if (hitL) {
			ind = leftIndex;
			}
			...
			if (ptr == 0) return false; // traversal end
			ind = stk[--ptr];
	}*/
			...
			/*if (left)
{
    // có left ? ban ??u ?i left
    int nearIndex  = leftIndex;
    int farIndex   = rightIndex;

    if (right)
    {
        // n?u right g?n h?n ? swap near / far
        bool swap = (tl2 < tl1);

        // branchless swap
        int t0 = int(swap);
        int t1 = 1 - t0;
        nearIndex = t1 * leftIndex  + t0 * rightIndex;
        farIndex  = t0 * leftIndex  + t1 * rightIndex;
...
else if (right)
{
    ...
    continue;
			...
			int next = hitL ? leftIndex : (hitR ? rightIndex : stk[--ptr]);
			if(hitL && hitR)
				stk[ptr++] = (tl1 > tl2 ? leftIndex : rightIndex);
