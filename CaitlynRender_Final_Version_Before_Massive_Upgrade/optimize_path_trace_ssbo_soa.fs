...
#define min_x(i) data[(i)]
#define min_y(i) data[nodeCount + (i)]
#define min_z(i) data[nodeCount*2 + (i)]

#define max_x(i) data[nodeCount*3 + (i)]
#define max_y(i) data[nodeCount*4 + (i)]
#define max_z(i) data[nodeCount*5 + (i)]

#define start(i) int(data[nodeCount*6 + (i)])
#define range(i) int(data[nodeCount*7 + (i)])
...
bool trace_bvh(Ray r, out HitRecord rec)
{
	...
	while(ind > -1)
	{
		...
		//int ind = n;
		//vec4 bmin = vec4(min_x[ind], min_y[ind], min_z[ind], start[ind]);
		//vec4 bmax = vec4(max_x[ind], max_y[ind], max_z[ind], range[ind]);

		int leftIndex = start(n);//int(bmin.w);
		int num_triangle = range(n);
		...
		if(num_triangle == 0)
		{
			...
			bool left  = (tl1 <= th1) && (th1 >= 0.0) && (tl1 < t);
			bool right = (tl2 <= th2) && (th2 >= 0.0) && (tl2 < t);
		...
		{
			//int num_triangle = range[n];//int(bmax.w);//int(info.y);
			
			int end = leftIndex + num_triangle;
...
bool hit_shadow(Ray r, float max_t)
{
	...
	while(ind > -1)
	{
		...
		if(num_triangle != 0)
		{
			//int num_triangle = range[n];//range = int(bmax.w);//int(info.y);

			
			for (int i = leftIndex; i < leftIndex + num_triangle; ++i) // Loop through indices
		...
		else
		{
			...
			bool left  = (tl1 <= th1) && (th1 >= 0.0) && (tl1 <= max_t);
			bool right = (tl2 <= th2) && (th2 >= 0.0) && (tl2 <= max_t);
...
void main1()
{
	vec3 bmin = vec3(min_x(0), min_y(0), min_z(0));
    vec3 bmax = vec3(max_x(0), max_y(0), max_z(0));
    float s = start(0);
    float r = range(0);

    // normalize to viewable range (you can tweak scale)
    vec3 c = clamp((bmin + bmax) * 0.5 * 0.1 + vec3(0.5), 0.0, 1.0); // center
    // overlay start/range into channels
    float sx = clamp(s / 255.0, 0.0, 1.0);
    float rx = clamp(r / 255.0, 0.0, 1.0);

    color = vec3(c.r, sx, rx);
}
