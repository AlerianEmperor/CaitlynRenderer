...
struct SBVH
{
	...
	void convert_to_soa()
	{
		for (auto& v : flat_nodes)
		{
			...
			min_x.emplace_back(bmin.x);
			min_y.emplace_back(bmin.y);
			min_z.emplace_back(bmin.z);

			start.emplace_back((int)bmin.w);

			max_x.emplace_back(bmax.x);
			max_y.emplace_back(bmax.y);
			max_z.emplace_back(bmax.z);

			range.emplace_back((int)bmax.w);
		}
		int N = flat_nodes.size();
		...
		soa.reserve(N *8);
		soa.insert(soa.end(), min_x.begin(), min_x.end());
		soa.insert(soa.end(), min_y.begin(), min_y.end());
		soa.insert(soa.end(), min_z.begin(), min_z.end());
		soa.insert(soa.end(), max_x.begin(), max_x.end());
		soa.insert(soa.end(), max_y.begin(), max_y.end());
		soa.insert(soa.end(), max_z.begin(), max_z.end());
		soa.insert(soa.end(), start.begin(), start.end());
		soa.insert(soa.end(), range.begin(), range.end());
	}
