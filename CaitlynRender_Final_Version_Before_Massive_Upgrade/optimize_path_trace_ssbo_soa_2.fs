...
void get_node_info(int node, inout vec3 mn, inout vec3 mx, inout int start, inout int range)
{
	int base0 = 0;//node_count * 0u;
	int base1 = node_count;
	int base2 = node_count * 2;
	int base3 = node_count * 3;
	int base4 = node_count * 4;
	int base5 = node_count * 5;
	int base6 = node_count * 6;
	int base7 = node_count * 7;
	...
	start = floatBitsToInt(bvh[base6 + node]);
	range = floatBitsToInt(bvh[base7 + node]);

	//uint left  = node * 2u;
	//uint right = node * 2u + 1u;
}
...
float hit_bbox(Ray r, int node_ind, vec3 invdir, out float tl)
{
	...
	get_node_info(node_ind, bmin, bmax, start, range);
...
bool trace_bvh(Ray r, out HitRecord rec)
{
	...
	while(ind > -1)
	{
		...
		vec3 dumb_min, dumb_max;
		int leftIndex, range;

		get_node_info(n, dumb_min, dumb_max, leftIndex, range);
